...
import { WaitlistCandidateDocument } from './schema';
...
export async function checkCandidatePosition(token: string): Promise<PositionInfo | null> {
  ...
  const sortedActive = [...activeCandidates].sort((a, b) => (a.index || 0) - (b.index || 0));
  const candIndex = candidate.index || sortedActive.findIndex(c => c.id === candidate.id) + 1;
  const peopleAhead = sortedActive.filter(c => (c.index || 0) < candIndex).length;
  const currentPos = peopleAhead + 1;
  const movedUp = Math.max(0, originalPos - currentPos);
...
export async function cancelCandidatePosition(token: string): Promise<Candidate | null> {
  const tokenClean = token.trim();
  if (!tokenClean) return null;
  ...
  if (db) {
    try {
      ...
      console.error('[MongoDB] Error cancelling position:', err);
    }
  }
...
export async function setCandidateVerificationCode(email: string, code: string): Promise<void> {
  const normalized = email.trim().toLowerCase();
  ...
  const localCandidate = localStore.candidates.find((c) => c.email.toLowerCase() === normalized);
  if (localCandidate) {
    localCandidate.verificationCode = code;
    localCandidate.codeExpiresAt = codeExpiresAt;
    saveLocalStore();
  }
}

export async function verifyCandidateCode(email: string, code: string): Promise<{
  success: boolean;
  ...
  if (db) {
    try {
      const doc = await db.collection<WaitlistCandidateDocument>('waitlist').findOne({ email: normalized });
      if (doc && doc.verificationCode === code && doc.codeExpiresAt && doc.codeExpiresAt > Date.now()) {
        const rawToken = generateSecureToken();
        const tokenHash = hashToken(rawToken);
        await db.collection('waitlist').updateOne(
          { email: normalized },
          { $set: { tokenHash, isVerified: true, status: 'ACTIVE', verificationCode: null, codeExpiresAt: null } }
        );
        const candidate: Candidate = {
          ...doc,
          id: doc.candidateId,
      ...
      console.error('[MongoDB] Error verifying code:', err);
    }
  }

  // Local fallback
  const localCandidate = localStore.candidates.find(
    (c) => c.email.toLowerCase() === normalized && c.verificationCode === code && c.codeExpiresAt && c.codeExpiresAt > Date.now()
  );

  if (localCandidate) {
    const rawToken = generateSecureToken();
    const tokenHash = hashToken(rawToken);
    localCandidate.tokenHash = tokenHash;
    localCandidate.isVerified = true;
    localCandidate.status = 'ACTIVE';
    localCandidate.verificationCode = undefined;
    localCandidate.codeExpiresAt = undefined;
    saveLocalStore();
  ...
  return { success: false, candidate: null, token: null, error: 'Invalid or expired verification code.' };
}